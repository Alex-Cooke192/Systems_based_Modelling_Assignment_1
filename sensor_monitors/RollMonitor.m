%% RollMonitor.m
% Health + conditioning state machine for roll angle (deg).
% States: OK, SUSPECT, FAILED
%
% Input: measured roll from RollSensorSITL (may be noisy/NaN/faulty)
% Output: conditioned roll, health state and diagnostic flags

classdef RollMonitor < handle
    properties
        % Timing
        Fs (1,1) double = 50
        dt (1,1) double = 0.02
        ...
        % Valid roll range
        MinDeg (1,1) double = -180
        MaxDeg (1,1) double =  180

        NearThreshBand (1,1) double = 10

        % Conditioning 
        LPFAlpha (1,1) double = 0.2
        MaxRateDegPerS (1,1) double = 180     % [deg/s] rate limiter
    ...
    methods
        function obj = RollMonitor(varargin)
            % Name-value init
        ...
        function [rollCond, state, flags] = step(obj, measuredRollDeg)
            dt = obj.dt;
